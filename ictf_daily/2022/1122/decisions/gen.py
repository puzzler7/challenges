...
flag.close()
os.system("zip -j -9 files/0/up-0.zip files/0/flag.txt")

flag = open("files/0/flag.txt", "w")
flag.write(open("flag.txt").read())
flag.close()
os.system("zip -j -9 files/0/down-0.zip files/0/flag.txt")
...
for i in range(1, levels):
    ...
    os.system(f"zip -j -9 files/{i}/{new_real}-{i}.zip files/{i-1}/up-{i-1}.zip files/{i-1}/down-{i-1}.zip")
    os.system(f"cp files/{i-1}/{not_last_real}-{i-1}.zip files/{i-1}/{last_real}-{i-1}.zip")
    os.system(f"zip -j -9 files/{i}/{not_new_real}-{i}.zip files/{i-1}/up-{i-1}.zip files/{i-1}/down-{i-1}.zip")
